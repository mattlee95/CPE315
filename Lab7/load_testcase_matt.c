...
/*Pipelined Instruction Stuff*/

/* Pipeline Restrictions
 * ALU cannot happen during other ALU
 * REG cannot happen during other REG
 * WRITE cannot happen during other WRITE
 * MEM cannot happen during other MEM
 * REG cannot happen until previous WRITE if using same register
*/
...
typedef struct
{
   /* keeps track of stages of pipeline being utilized at certain times */
   /* f - fetch, r - read, e - execute, m - memory access, w - write */
   instruct array[1000];
   /* number of array associated with writes */
   int instruct_index;
   int clock_index;
} pipeline;
...
/* types of instructions r-reg, i-imm, b-branch, j-jump, m-mem */
/* returns how many extra cycles pipeline restrictions will cause */
int add_instruct(pipeline *pipe, char instruct_type, unsigned int Rd, unsigned int Rt, unsigned int Rs)
{
   //determine location to start instructoin
   //determine if any stages conflict
   //determine if write to source reg comes after reg read
   int extra_cycles = 0, i;
   if (instruct_type == 'r') {
      // f, r, e, w
      // dest = rd
      // source = rs, rt
      for (i = pipe->instruct_index-1; i >= 0; i--)
      {
         if ((pipe->array[i].w_loc <= pipe->clock_index) || (pipe->array[i].w_loc != -1))
         {
            //break if instruction write no longer interferes with incruction location
            break;
         }
         else
         {
            if ((pipe->array[i].dest_reg == Rt) || (pipe->array[i].dest_reg == Rs))
            {
               extra_cycles = pipe->array[i].w_loc - pipe->clock_index;
               pipe->clock_index = pipe->array[i].w_loc;
            }
         }
      }
      pipe->array[pipe->instruct_index].f_loc = pipe->clock_index;
      pipe->array[pipe->instruct_index].r_loc = pipe->clock_index+1;
      pipe->array[pipe->instruct_index].e_loc = pipe->clock_index+2;
      pipe->array[pipe->instruct_index].m_loc = -1;
      pipe->array[pipe->instruct_index].w_loc = pipe->clock_index+3;
      num_fetch++; num_reg++; num_ex++; num_write++;
      pipe->array[pipe->instruct_index].dest_reg = Rd;
      pipe->clock_index += 1;
      pipe->instruct_index += 1;
   }
   if (instruct_type == 'i') {
      // f, r, e, w
      // dest = rt
      ...
      pipe->array[pipe->instruct_index].w_loc = pipe->clock_index+3;
      num_fetch++; num_reg++; num_ex++; num_write++;
      pipe->array[pipe->instruct_index].dest_reg = Rt;
      pipe->clock_index += 1;
      pipe->instruct_index += 1;
   } 
   if (instruct_type == 'b') {
      ...
      pipe->array[pipe->instruct_index].e_loc = pipe->clock_index+2;
      pipe->array[pipe->instruct_index].m_loc = -1;
      pipe->array[pipe->instruct_index].w_loc = -1;
      num_fetch++; num_reg++; num_ex++;
      pipe->array[pipe->instruct_index].dest_reg = -1;
      pipe->clock_index += 1;
      pipe->instruct_index += 1;
   } 
   if (instruct_type == 'j') {
   ...
   if (instruct_type == 'm') {
      ...
      pipe->array[pipe->instruct_index].r_loc = pipe->clock_index+1;
      pipe->array[pipe->instruct_index].e_loc = pipe->clock_index+2;
      pipe->array[pipe->instruct_index].m_loc = pipe->clock_index+3;
      pipe->array[pipe->instruct_index].w_loc = pipe->clock_index+4;
      num_fetch++; num_reg++; num_ex++; num_mem++; num_write++;
      pipe->array[pipe->instruct_index].dest_reg = Rt;
      pipe->clock_index += 2;
      pipe->instruct_index += 1;
   }
...
void j(MIPS instruction) {
   ...
   unsigned int jump_address = (instruction) & 0x0000FFFF;
